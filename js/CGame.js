function CGame(oData) {
    ...
    window.socket.on("gameEnd", () => {
        this.gameOver(WHITE)
    });

    window.socket.on("changePieceBy", (data) => {
        var newPos = {
            row: 7 - data.pos.row,
            col: data.pos.col
        }
        ...
        if (_iCurPlayer === WHITE) {
            ...
            this.changePiece(data.piece, newPos);
        }
        this.changeTurn()
    })
    ...
    function convertMoveToOpponentPerspective(startRow, startCol, endRow, endCol) {
        // Convert the rows based on the mirroring logic
        let opponentStartRow = 7 - startRow;
        let opponentEndRow = 7 - endRow;

        // Columns remain the same
        let opponentStartCol = startCol;
        let opponentEndCol = endCol;

        return {
            opponentStart: { row: opponentStartRow, col: opponentStartCol },
            opponentEnd: { row: opponentEndRow, col: opponentEndCol }
        };
    }
    ...
    this.onFinishMove = function () {
        ...
        if (oPawnPos === null) {
            ...
            // } else {
            if (_iCurPlayer === WHITE) {
                PromoPanel = new CPromoPanel(_iCurPlayer, oPawnPos);
            }
    ...
    this.gameOver = function (iWinner) {
        ...
        if (iWinner === WHITE) {
            ...
            window.parent.postMessage({ type: "win_chess", winner: PLAYER_ID }, "*");
